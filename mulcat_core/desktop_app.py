...
from mulcat_core.platforms import PlatformAdapter
from mulcat_core.profile_manager import (
    APP_DIR,
    SCRIPTS_DIR,
    ProfileError,
    delete_profile,
...
class DesktopApi:
    ...
    def save_script(self, kind: str, profile_id: str, text: str) -> dict:
        with self.lock:
            try:
                ...
                path.write_text(str(text), encoding=self.adapter.script_encoding())
                self.adapter.after_script_write(path)
    ...
    def save_script_and_sync(self, kind: str, profile_id: str, text: str) -> dict:
        with self.lock:
            try:
                ...
                self.adapter.after_script_write(script_path(kind, profile_id))
    ...
    def launch_profile(self, kind: str, profile_id: str) -> dict:
        with self.lock:
            path = ensure_script(kind, profile_id)
            result = self.adapter.launch_script(path, APP_DIR, profile_id)
            self.last_message = result.message
            return self._state()

    def open_scripts_dir(self) -> dict:
        self.adapter.open_directory(SCRIPTS_DIR)
        return self._state()

    def copy_text(self, text: str) -> dict:
        with self.lock:
            try:
                self.adapter.copy_text(str(text))
...
def run_desktop(adapter: PlatformAdapter) -> None:
    api = DesktopApi(adapter)
